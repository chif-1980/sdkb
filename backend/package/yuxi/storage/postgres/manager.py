    ...
    async def ensure_knowledge_schema(self):
        ...
        stmts = [
            ...
            CREATE TABLE IF NOT EXISTS feishu_source_items (
                ...
                title VARCHAR(512),
                parent_item_key VARCHAR(512),
                path_text VARCHAR(2048),
                source_url VARCHAR(2048),
                last_seen_at TIMESTAMPTZ,
                source_updated_at TIMESTAMPTZ,
                source_validity VARCHAR(32) NOT NULL DEFAULT 'valid',
                active_version_id VARCHAR(64),
                created_at TIMESTAMPTZ DEFAULT NOW(),
                updated_at TIMESTAMPTZ DEFAULT NOW(),
                CONSTRAINT uq_feishu_source_items_item_key UNIQUE (item_key)
            )
            """,
            """
            CREATE TABLE IF NOT EXISTS feishu_material_versions (
                id SERIAL PRIMARY KEY,
                ...
                parsed_object_path VARCHAR(1024),
                ...
                created_at TIMESTAMPTZ DEFAULT NOW(),
                updated_at TIMESTAMPTZ DEFAULT NOW(),
                CONSTRAINT uq_feishu_material_versions_identity UNIQUE (item_id, revision, content_hash)
            )
            """,
            """
            CREATE TABLE IF NOT EXISTS feishu_processing_events (
                id SERIAL PRIMARY KEY,
                source_id VARCHAR(64) NOT NULL REFERENCES feishu_sources(source_id) ON DELETE CASCADE,
                item_id VARCHAR(64) REFERENCES feishu_source_items(item_id) ON DELETE CASCADE,
                version_id VARCHAR(64) REFERENCES feishu_material_versions(version_id) ON DELETE CASCADE,
