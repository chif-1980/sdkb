<template>
  <section class="governance-card">
    ...
    <div class="comparison-status" :class="`status-${comparisonStatus.status}`">
      ...
      <span v-if="comparisonStatus.total"
        >{{ comparisonStatus.completed }}/{{ comparisonStatus.total }} 份资料</span
      >
      <span v-if="comparisonStatus.relation_count"
        >· {{ comparisonStatus.relation_count }} 条关系</span
      >
      <span v-if="comparisonStatus.status === 'completed' || comparisonStatus.issue_count"
        >· {{ comparisonStatus.issue_count || 0 }} 条待处理</span
      >
      <span v-if="comparisonTask?.message" class="status-message"
        >· {{ comparisonTask.message }}</span
      >
    </div>
    <div class="toolbar">
      ...
      <template #bodyCell="{ column, record }">
        <template v-if="column.key === 'relation_type'">
          <div class="relation-cell">
            <a-tag :color="relationColor(record.relation_type)">{{
              relationLabel(record.relation_type)
            }}</a-tag>
            <span
              class="relation-signal"
              :class="`signal-${record.relation_type?.toLowerCase()}`"
              >{{ relationSignal(record.relation_type) }}</span
            >
          </div>
        </template>
        <template v-else-if="column.key === 'topic'"
          ><strong class="primary-cell">{{ record.source_title }}</strong
          ><span class="sub-line">{{ record.reasoning || '等待人工确认关系' }}</span></template
        >
        ...
        <template v-else-if="column.key === 'evidence'">
          <div class="evidence-inline">
            <span v-if="record.different_content?.length" class="evidence-chip evidence-chip-diff"
              >差异 {{ record.different_content.length }}</span
            >
            <span v-if="record.same_content?.length" class="evidence-chip evidence-chip-same"
              >相同 {{ record.same_content.length }}</span
            >
            <span
              v-if="!record.different_content?.length && !record.same_content?.length"
              class="sub-line"
              >待补充证据</span
            >
          </div>
        ...
        <template v-else-if="column.key === 'scope'"
          ><span>{{ record.scope_difference?.summary || '范围待确认' }}</span></template
        >
        <template v-else-if="column.key === 'status'"
          ><a-tag :color="relationStatusColor(record)">{{
            relationStatusLabel(record)
          }}</a-tag></template
        >
        <template v-else-if="column.key === 'action'">
          <a-button size="small" @click="$emit('open-review', record)">{{
            relationActionLabel(record)
          }}</a-button>
        </template>
      </template>
    ...
    <div class="table-footer">
      当前显示 {{ filteredRelations.length }} 条 · 全部
      {{ comparisonStatus.relation_count || 0 }} 条 · 待处理
      {{ comparisonStatus.issue_count || 0 }} 条
    </div>
  </section>
</template>
...
const filteredRelations = computed(() => {
  ...
  return relations.value.filter((item) =>
    `${item.source_title} ${item.target_title} ${item.reasoning || ''}`
      .toLowerCase()
      .includes(keyword)
  )
})
...
const emptyDescription = computed(() => {
  ...
  if (comparisonStatus.value.status === 'queued' || comparisonStatus.value.status === 'running')
    return '检查进行中，完成后会显示关系'
  if (comparisonStatus.value.status === 'failed') return '检查失败，请点击“补跑检查”重试'
  return '已完成检查，暂未发现需要人工裁决的问题'
})

watch(() => [props.sourceId, relationType.value, relationStatus.value], loadRelations, {
  immediate: true
})
...
async function loadRelations() {
  ...
  try {
    const [response, status] = await Promise.all([
      governanceApi.listRelations(props.sourceId, {
        relation_type: relationType.value || undefined,
        status: relationStatus.value || undefined
      }),
    ...
    emit('count-change', comparisonStatus.value.relation_count || 0)
  } catch (error) {
  ...
  taskPollTimer = setTimeout(async () => {
    try {
      ...
      if (['success', 'failed', 'cancelled'].includes(comparisonTask.value.status)) {
        backfillLoading.value = false
        await loadRelations()
        if (comparisonTask.value.status === 'failed')
          message.error('跨文档检查失败，请查看任务详情后重试')
      } else {
        pollComparisonTask()
      }
    } catch {
...
function relationSignal(value) {
  return (
    {
      CONFLICT: '需裁决',
      OVERLAP: '有共同内容',
      EXACT_DUPLICATE: '已识别重复',
      CONDITIONAL_VARIANT: '范围不同',
      INSUFFICIENT: '证据不足'
    }[value] || '待确认'
  )
}
function relationRowClass(record) {
  return `relation-row-${String(record.relation_type || '').toLowerCase()}`
}
function relationNeedsAction(record) {
  return record.status === 'open' && ['CONFLICT', 'INSUFFICIENT'].includes(record.relation_type)
}
function relationStatusLabel(record) {
  return record.status !== 'open' ? '已处理' : relationNeedsAction(record) ? '待处理' : '已识别'
}
function relationStatusColor(record) {
  return record.status !== 'open' ? 'success' : relationNeedsAction(record) ? 'error' : 'default'
}
...
.governance-card :deep(.relation-row-conflict > td) {
  background: #fff8f8;
}
.governance-card :deep(.relation-row-conflict > td:first-child) {
  box-shadow: inset 3px 0 0 #d45a5f;
}
.governance-card :deep(.relation-row-overlap > td) {
  background: #fffdf4;
}
.governance-card :deep(.relation-row-overlap > td:first-child) {
  box-shadow: inset 3px 0 0 #d6aa36;
}
.governance-card :deep(.relation-row-exact_duplicate > td:first-child) {
  box-shadow: inset 3px 0 0 #68a57b;
}
.primary-cell {
  display: block;
  color: var(--color-text);
