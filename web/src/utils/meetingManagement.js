const labels = {
  ...
  TASK_SAVE: '保存待办', TASK_CONFIRM: '确认待办', TASK_RESEND: '重试飞书同步', TASK_IGNORE: '忽略待办', TASK_RECONCILE: '核对并恢复飞书关联',
  TASK_APPLY_AI: '采用待办修改建议', TASK_DISMISS_AI: '拒绝待办修改建议',
...
// Completion readback and schedule agreement are separate facts.
export const taskScheduleLabel = delivery => {
  if (delivery?.pendingUpdate) return '本地修改待同步'
  if (delivery?.syncStatus === 'FAILED') return '飞书读取失败'
  return ({MATCH:'负责人、期限一致', DIFFERENT:'安排有差异', UNVERIFIED:'安排未核实'})[delivery?.scheduleComparison?.status] || '安排待核对'
}
