...
const modelManageTabs = computed(() => {
  ...
  if (userStore.hasPermission('models.view')) tabs.push({ key: 'providers', label: '模型供应商' })
  return tabs
})
...
const normalizeTab = (tab) => {
  if (tab === 'providers' && userStore.hasPermission('models.view')) return 'providers'
  return 'agents'
}

watch(
  () => [route.query.tab, userStore.hasPermission('models.view')],
  ([tab]) => {
...
<template>
  <div class="model-manage-view">
    ...
    <div class="model-manage-content">
      ...
      <div
        v-if="userStore.hasPermission('models.view') && activeTab === 'providers'"
        class="tab-panel"
      >
