...
## 0.7.0 / 0.6.10 飞书统一登录、嵌入与升级核对

善达知枢管理端 0.7.0 与企业知识助手 0.6.10 共用飞书应用和后端身份绑定。除飞书应用凭据外，部署时需核对以下配置：
...
管理端的“企业知识助手”菜单通过一次性、60 秒有效的登录交接码进入助手，不会把管理端令牌放进地址栏。交接成功后助手端写入独立的 HttpOnly 会话 Cookie；交接失败或过期时回到助手登录页。助手端仍会重新校验账号状态和部门归属。管理端在保留左侧菜单的右侧内容区加载助手页面，不会整页替换管理端路由。

统一 RBAC 由管理端后端最终鉴权：主菜单可见性、页面内操作、数据范围、用户直配角色和飞书部门角色分别计算，前端隐藏入口不能替代接口权限。超级管理员为受保护内置身份；首次飞书登录只登记账号，不自动授予管理权限。角色权限保存、用户授权、部门映射和撤销均写入操作审计。
...
1. 备份数据库和本地配置，配套更新 `sdkb` 0.7.0 与 `sdAssit` 0.6.10 的应用服务，保留现有数据卷和密钥。
2. 验证本地超级管理员仍可登录，再验证已授权管理角色的飞书账号登录。正常升级保留账号；清空数据库或改用新数据库不会自动保留原账号。
3. 在助手提交脱敏演示会议，核对后台状态与管理端“会议管理”的企业范围、纪要、待办及知识建议；再从管理端打开右侧嵌入助手，确认左侧菜单仍可操作。
4. 对明确用于验收的事项执行确认发送，分别核对飞书通知和待办是否成功；改派后应更新原任务，日期应与界面一致。应用还需具备相应通讯录、消息及任务权限，并发布权限变更。
5. 验证历史会议引用、正文修订、Word 导出和用户反馈闭环；知识建议经维护人员处理后，仍通过既有知识审核发布流程生效。

6. 验证角色权限：用普通用户、管理员和超级管理员分别登录，检查菜单显示、页面内写操作、数据范围、飞书部门继承及撤销即时生效；检查“创建新对话、搜索对话、账户设置”等固定入口不受角色隐藏配置影响。
...
开发环境（`YUXI_ENV=development` 且未设置该变量）默认允许 `http://localhost:5173` 与 `http://127.0.0.1:5173`，方便本地前后端独立启动调试。这里的 0.7.0 指善达知枢产品版本；底层 Yuxi 包的 0.7.1 兼容性规则仍按后端依赖执行。从底层 Yuxi 0.7.0 升级到 0.7.1 时，如果此前是跨域部署但未显式声明来源，必须补上 `YUXI_CORS_ORIGINS`，否则前端跨域请求会被拒绝。
