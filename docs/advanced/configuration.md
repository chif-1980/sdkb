...
数据源 API 和数据库业务配置不保存或公开凭据变量名、App ID、App Secret 或 tenant token 正文。新建数据源在兼容列写入固定内部标记 `GLOBAL_FEISHU_APP`；升级前已有历史值原样保留，但认证流程不再读取这些值，数据源 API 也不返回该列。真实凭据只保存在本机环境文件中，不得写入 TOML、日志、事件记录或版本库。
