...
本页保留阶段 1 的部署基线；知枢 0.7.0、企业知识助手 0.6.10 的飞书统一登录、会议管理、RBAC 和升级核对请参阅 [生产部署指南](../advanced/deployment.md)，当前操作以[助手使用手册](../guide/knowledge-assistant.md)和[知枢使用手册](../guide/zhishu-manual.md)为准。正式凭据只保存在部署机器的本地 `.env` 或密钥管理系统中，不写入仓库、命令参数、日志或验收记录。
