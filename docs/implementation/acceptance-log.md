...
- 离线端到端测试：4 个用例覆盖三层知识树完整扫描、PDF/TXT/PNG 处理、音频/视频发现后标记为不支持、审核、发布和飞书引用；增量 v2 替换及发布前 v1 持续生效；权限与局部遍历失败、429 重试、不支持媒体和日志令牌脱敏；Milvus 强制失败时保留 v1，并将 v2 标记为 `publish_failed`
