...
同一套共享后端中，善达知枢和企业知识助手使用页面顶部的“统一模型配置”。先在供应商中配置接口、凭证并启用所需模型，再按用途选择并点击“保存配置”。仅添加供应商或模型，不会自动切换业务调用。

| 用途 | 影响范围 |
| --- | --- |
| 统一业务模型 | 知识加工中的图谱、思维导图等，以及知识问答、会议纪要、方案和智能体 |
| 快速响应 | 标题生成等轻量任务 |
| 向量化 | 新建知识库的默认模型；已有索引继续使用原模型 |
| 检索重排 | 对检索结果按相关性排序 |
| 内容审查 | 仅在系统启用模型审查后使用 |

切换模型后，以新建问答、会议分析或加工任务验证实际调用。已有成功结果不会自动重写，运行中的任务也不应被视为已切换。若提示模型不可用或未授权，检查供应商启用状态、凭证、模型 ID 和账号的模型访问权限；已有向量索引更换模型需单独评估并重建索引，不能混用不同向量空间。
