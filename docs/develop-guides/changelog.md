...
- 完成飞书知识加工与治理闭环：知识源读取改为企业自建应用下的用户 OAuth 授权，管理页支持扫码与浏览器授权、空间根目录树缓存、全量/增量扫描及来源失效下架；扫描范围覆盖 Wiki/Docx、PDF、Word、PowerPoint、Excel 和图片 OCR，音视频暂只登记来源。解析后新增跨文档比较、重复/重叠/冲突/证据不足关系、发布/需要修改/驳回/转交审核决策、适用范围、正式知识版本和来源关系，并将审核工作台、关系检查和正式知识直接整合进原有飞书知识加工页面。
- 新增面向业务用户的独立企业知识助手：使用飞书 OAuth 登录，固定检索指定的正式飞书知识源，支持 `SUPPORTED`、`INSUFFICIENT`、`CONFLICTING` 三态回答、权限内引用及来源跳转；问答界面与 Yuxi 技术工作台隔离，提供流式处理链提示、答案点赞/点踩、长页面回到底部和移动端来源抽屉，并补充会话、引用、权限边界及真实知识链路的自动化测试和运维说明。
