...
- 新增八张交互图：核心功能、业务、技术架构，会议业务与知识治理流程，助手问答与飞书任务同步时序，身份与 RBAC 生命周期。
- 系统架构页、首页及两份使用手册统一使用新图入口与预览，支持放大、聚焦、搜索和图片导出。
- 修复独立图示链接被文档路由拦截的问题，并兼容当前构建工具的页面脚本输出，避免文档加载时出现脚本 404。
- 区分当前实现与规划，明确回读批次间隔、未发送修改的跳过规则、反馈运营边界及管理端飞书登录身份限制。本次不修改业务代码或产品版本。

## 善达知枢 0.7.0（2026-09-24）

- 统一 RBAC：支持自定义角色、内置管理员和普通用户默认权限、用户授权及飞书部门继承；角色工作区按主菜单组织，包含菜单与操作、数据范围、授权成员和生效预览。
- 权限保存支持冲突检测和审计，保护超级管理员身份；用户反馈支持本人、部门和全企业范围，其他模块当前支持无权限或全企业。
- “企业知识助手”在知枢右侧内容区打开并保留左侧菜单，使用一次性、60 秒有效的登录交接码，无需再次登录；助手独立会话继续检查账号状态和部门归属。
- 管理端生产 CSP 放行助手域名，保留管理端自身的防嵌入限制；部署前需核对助手端嵌入策略。
- 产品标签及管理端 `/version.json` 为 `0.7.0`，配套助手沿用 `0.6.10`；底层 Yuxi 包版本保持原约定。
- 在线文档按 0.7.0 / 0.6.10 重新核对：更新首页、管理员手册、助手手册和部署说明，补充统一 RBAC、飞书部门角色、右侧助手嵌入、用户反馈和会议同步；替换系统架构图并新增脱敏的角色权限、嵌入助手、助手首页和会议纪要界面示意。

发布验证：Docker 内 83 项后端及部署策略单元测试、2 项数据库/API 集成测试、18 项管理端测试通过；Ruff、ESLint 和管理端生产构建通过。配套助手 363 项测试通过。服务器更新需另行部署。

### RBAC 变更明细

- 超级管理员可以编辑内置“管理员”和“普通用户”的默认权限；内置超级管理员权限固定，内置身份不可删除或作为额外角色绑定。默认模板仅用于没有直接或部门角色的账号；明确保存全无权限时不再回退到旧管理员授权。已有自定义授权保持原规则，修改支持冲突检测和审计。

- 角色权限按确认原型改为独立工作区：设置入口打开角色列表、菜单与操作、数据范围、授权成员、生效预览；主菜单名称与顺序共用定义。用户与飞书部门支持搜索和分页，直接授权与部门继承分开说明。
- 权限预览使用与后端鉴权相同的解析结果，展示直接角色、部门继承和旧管理员兼容规则；新增保存快照冲突校验、内置身份保护、父子菜单校验、反馈管理范围校验和审计。
- 保存、删除和授权确认改为页面内对话框；授权失败或取消回退勾选，未保存草稿离开时提示。知识库和模型供应商页签按能力权限显示。
- 验证：15 项前端测试、27 项后端专项测试及 1 项真实运行 API 集成测试通过，后者覆盖新建角色、保存、并发快照冲突、用户绑定、真实预览、撤销及即时拒绝访问；测试数据已清理。

- 修复助手点踩反馈生成知识复核任务时的父子记录写入顺序，避免 PostgreSQL 外键错误导致反馈回滚；新增真实 PostgreSQL 回归，覆盖“其他＋补充说明”、重复提交和后台可见性。
- 在会议管理下方新增独立的“用户反馈”菜单和页面，直接查看全部、点赞、点踩反馈；数据总览中的原反馈入口继续可用。
- 将反馈专用权限配置扩展为统一 RBAC：支持自定义角色、模块查看/管理权限、本人/部门/全企业数据范围、用户角色指派和飞书部门角色映射；按已同步飞书部门成员关系实时继承授权，权限变更写入操作日志。
- 管理路由和关键页面切换为权限矩阵判断，传统超级管理员/管理员继续兼容；超级管理员能力不可通过普通角色授予，用户 CRUD 不能借机提升系统账号角色。
- 完善角色权限页面的范围说明、用户直配角色和飞书部门角色说明，明确菜单可见、页面操作和部门继承的关系。

## 善达知枢 0.6.5（2026-09-23）

- 修复助手点踩选择“其他”并补充说明时反馈提交失败的问题，确保反馈、知识复核任务和通知记录按外键依赖顺序写入并支持重复提交幂等处理。
- 在会议管理下新增独立用户反馈入口；补充反馈查看范围权限配置和审计记录。
- 验证：PostgreSQL 反馈回归 1 项、生命周期与反馈权限相关单元测试 21 项通过。

## 善达知枢 0.6.4（2026-09-21）

- 首期飞书工作台嵌入：企业知识助手桌面端和移动端入口统一指向助手地址，客户端内通过飞书授权登录并保留安全会议深链接；登录状态沿用现有产品会话、权限和会话数据。
- 会议完成通知改为独立持久化投递记录，按上传者的有效飞书企业绑定发送卡片，包含会议直达链接；失败按后台计划重试，复用同一幂等 UUID，账号撤权、企业变化或会议归属变化时阻止发送。
- 通知配置 `FEISHU_MEETING_COMPLETION_NOTIFICATIONS` 默认关闭，仅新完成会议入队，不补发历史会议；开启前必须完成真实收件人验收。
- 验证：助手前端登录/深链测试 25 项通过；后端认证、会议通知、飞书客户端及接口测试 147 项通过；变更文件 Ruff 检查通过。飞书开放平台版本发布和真实收件人通知仍需人工验收。

## 善达知枢 0.6.3（2026-09-20）

- 审核正文和新旧版本对照按各版本文件实际所属知识库读取，不再误用知识源当前发布目标库。切换知识源目标库后，已有解析正文仍可查看；正文记录缺失时明确提示检查解析结果。
- 保留原发布目标和知识数据，不迁移文件、不重新扫描或自动发布。
- 验证：知识治理 123 项、审核页面 60 项及正文定位/版本接口专项 5 项测试通过，Ruff、ESLint 和管理端构建通过。扩展接口回归发现 9 项既有失败，已在修改前的 `83ecaf69` 代码复现，主要涉及旧审核流程断言及新增授权路由清单；另 1 项跳过，本次未改动这些无关流程。

## 善达知枢 0.6.2（2026-09-20）

产品标签与管理端 `/version.json` 统一为 `0.6.2`，配套企业知识助手 `0.6.2`；底层 Yuxi 包及依赖版本保持原约定。发布不代表真实飞书写入及大批量同步验收已完成。

发布前检查：助手全量 346 项测试、后端相关 184 项测试、管理页面 10 项测试通过；类型检查、Ruff、ESLint 和双端构建通过。部署状态以服务器版本及健康检查为准。

- 飞书任务回读增加负责人、期限核对；管理待办列表、详情及助手显示差异、两边的值和核对时间，可打开原任务。完成状态独立同步，不自动覆盖双方安排；读取失败保留上次快照并标注尚未重新核实。全天日期按 UTC 日历日期解释，具体时间标注北京时间；响应不完整时显示未核实。本地未发送修改继续跳过回读，发送成功后等待重新核对。
- 修改待办标题、负责人或期限时，不再覆盖飞书端已完成的状态；只有显式修改执行状态才写入飞书，保存和失败重试保留该意图。
- 重复确认不重复更新飞书截止日期；已有任务读取失败后重试必须真实回读，不再仅凭任务编号标记同步成功。任务创建仍只使用飞书待办通知。
- 修正飞书创建任务的 `client_token`：按官方接口要求放入请求正文，避免超时或 500 后自动重试重复创建；补充响应丢失及本地事务回滚后的同参数立即重试验证。客户端幂等保障限于飞书官方 5 分钟窗口，跨请求防重复由下述持久化发送记录承担。
- 新增独立提交的飞书任务创建记录：先保存创建意图，成功后单独保存任务编号；会议保存回滚后仍可恢复原任务，修改内容后重试更新原编号。结果不明、应用身份变化及历史无编号失败任务显示“待核对”并停止重复创建，不再依赖 5 分钟窗口进行跨请求重发。管理员恢复入口见下项，无法证明来源的历史任务继续阻止重建。
- 会议管理 → 待办事项新增“核对飞书原任务”：填写任务详情链接或编号，读取来源、内容、负责人编号、期限及完成状态，填写核对说明后恢复关联。新任务写入唯一来源标记；仅来源标记或已有成功回执匹配才能恢复。企业身份、应用、版本、重复关联均校验；回执、会议、助手投影及审计在同一事务保存。恢复仅读取飞书，不创建、修改任务或发送通知；本地与远端不一致时保留本地安排并标记“待同步修改”。缺少来源证明的旧任务仍需人工核查，不能强行解除防重复保护。
- 同一会议后续任务读取超时时，保留已经读取成功的任务状态，并标记剩余任务失败。
- 会议待办保存、确认、忽略、AI 建议处理及重试记录独立操作类型、操作人、结果和修改前后值；管理端处理记录显示对应中文名称和操作人。
- 验证普通用户管理权限、角色撤销即时生效、助手令牌不能用于管理端及跨企业/跨用户会议隔离。
- 正式知识检索白名单和历史引用统一排除已下线、恢复失败的来源。
- 本轮 Docker 内 141 项后端测试、10 项管理页面测试通过，另有 24 项助手会议卡片测试通过；两端构建与检查通过。浏览器隔离样例验证恢复和失败展示；另对真实会议执行只读核对，页面正确区分一条安排一致、一条本地 9 月 19 日与飞书 9 月 20 日的期限差异。未修改飞书安排或发送通知；真实创建/改派端到端与生产部署验收尚未完成。
