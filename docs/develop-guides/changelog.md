...
- 补齐已发布知识生命周期治理：正式知识支持知识单元下架、恢复、负责人及有效期维护，来源资料支持整篇下架、恢复和回滚到已审核历史版本；索引重建成功后才切换生命周期和活动版本，失败时保留原状态与旧索引。知识修订继续回到飞书源文档处理，重复发起复用开放任务；企业知识助手收到“不满意”反馈时会根据回答引用定位知识单元并生成去重的纠错任务。
