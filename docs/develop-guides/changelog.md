...
## 善达知枢 0.6.1（2026-09-20）

- 与企业知识助手 0.6.1 配套发布，统一会议待办、复制和导出的任务数据，保留 AI 修改人工核对。
- 修复飞书任务重复通知、截止日期偏移和完成状态回读，完善管理端状态展示与首次扫码后的授权提示。
- 助手优化手动编辑、AI 修改及目标会议提示；复制成功后按钮显示“已复制”，3 秒后恢复。
- 底层 Yuxi 包与镜像版本沿用原约定，本次产品版本使用 Git 标签 `0.6.1`。

## 2026-09-20 会议行动清单与待办统一

- 结构化待办作为唯一任务数据源，助手在纪要中展示同一待办入口；复制及 Word 导出按当前保存的负责人、期限和状态生成表格。
- 手动正文编辑不再提供重复任务表格；历史纪要存在结构化待办时更新派生表格，无结构化待办时保留原文。
- AI 修改已有待办先产生待核对建议，采用后仍需显式同步飞书，不覆盖任务身份或自动发送。

## 2026-09-20 飞书待办定时回读验证

- 飞书身份失效、企业身份不匹配或同步超时时，待办显示同步失败及原因，保留上次有效状态；后续同步成功后自动恢复。
- 校验飞书返回的完成状态，避免异常数据被误判为已完成或重新打开。重复读取相同状态不新增纪要修订，未发送的本地修改不被覆盖。
- 补充真实数据库与两端接口集成测试，覆盖完成、重新打开、失败恢复、并发互斥、归档跳过及分批调度；测试使用模拟飞书响应，不修改真实飞书任务。
- 确认并发送待办时只使用飞书任务助手通知，不再额外发送“sd知识库”机器人消息，避免同一待办出现两条通知。
- 修正日期型待办发送到飞书时的时区转换，知枢选择的截止日期与飞书显示日期保持一致。

## 2026-09-20 会议待办展示一致性

- 助手待办日期框自动带入原文中明确的完整日期，保留原文供核对；保存后记住人工修改或清空，避免再次带入。日历置灰并禁选今天之前的日期，保留已存在的历史期限。

- 待办列表将确认状态与执行进度分列，使用无边框符号与文字区分；未进入执行的事项弱化为灰色，进行中、已完成及异常分别使用蓝、绿、红色，详情页沿用同一视觉规则。

- 后台会议详情与待办队列补充原文识别的负责人、期限及待核对标记，区分待确认与执行状态。
- 返回页面或切入待办时读取最新数据；编辑期间保留草稿。纪要原文保留会议当时的安排。

## 2026-09-20 飞书首次登录与授权提示

- 无管理权限时明确提示飞书身份已验证、账号已登记，以及管理员授权入口和重新登录步骤。
- 用户管理分别展示飞书关联状态和管理权限是否开通，沿用现有角色和部门访问范围，不自动授予权限。

## 善达知枢 0.6.0（2026-09-19）

- 新增会议管理：企业范围会议列表、纪要修订、待办跟进及知识建议处理。
- 统一飞书登录身份与知识授权复用，管理权限独立校验，保留本地超级管理员。
- 完善待办修改、飞书任务同步、原文依据和正式知识比对；历史会议支持搜索、分页及版本选择。
- 与企业知识助手 0.6.0 配套发布；底层 Yuxi 包与镜像版本保持原约定。
- 同步双仓库 README、文档首页及两端使用手册，补齐待办发送与修改、知识比对、历史会议选择、会议管理及统一登录配置说明。

## 2026-09-19 飞书统一登录与知识授权复用

- 知枢新增飞书登录，与企业知识助手共享身份；管理角色单独校验，保留本地账号登录。
- 登录通过绑定浏览器的一次性中间码完成，限制回跳地址，避免在 URL 中传递管理令牌。
- 新增“飞书账号与授权”入口；知识源可复用同一飞书身份的已有授权，实时核验企业及目标资料访问权，共享凭据统一刷新。

## 2026-09-19 会议管理（分阶段实现，本地验证）

- 会议管理按接口返回的实际权限范围显示提示；未关联飞书企业的管理账号明确提示仅可查看本人会议，避免把权限筛选后的空列表误认为会议未保存。
- 管理端新增会议列表、待办跟进和知识建议入口；支持分页搜索、纪要编辑、证据查阅、分析修订、独立归档和 Word 导出。管理员完成飞书企业授权后可查看同企业全部成员会议，并按企业隔离；未绑定企业时兼容显示本人会议。
- 显式重跑使用稳定会议编号，失败后保留最近成功结果；保留人工待办、知识处理决定和飞书任务编号，历史导入不触发外部发送。
- 飞书待办修改更新原任务，确认状态与同步失败分开；支持手动和定时读取完成状态，未同步的本地修改不被覆盖。状态未变化时不新增纪要修订。
- 知识维护支持引用正式知识并标记已覆盖、提交已有正式知识的来源修改审核、登记新增知识待补回原文、暂缓和不采纳；修改建议进入现有来源变更、审核与发布流程，不直接标记为已发布。
- 提供同权限范围的待确认、逾期、待核对和分析异常指标。企业范围查询与已有知识来源修改审核已接入，本地浏览器和生产环境验收仍需继续完成。

## 2026-09-19 历史会议引用选择

- 历史会议支持按标题、纪要关键词搜索和分页，展示来源、分析时间及纪要预览。
- 同源链接或明确重跑关系合并为分析版本，默认最新成功结果；同名不同来源保持独立。
- 已选项固定展示，搜索翻页后保留选择；同一会议改选版本会替换原选择，最多引用 5 场。

## 2026-09-19 会议知识比对状态提示

- 区分未执行检索、检索失败、缺少可引用依据、比对生成失败和单条结论缺失，避免统一提示重新分析。

## 2026-09-18 会议待办飞书发送修复

- 飞书通知和待办统一使用企业成员 ID，严格校验业务响应与消息、待办编号，失败时明确提示。
- 已确认事项可重新发送通知并复用原待办；重发失败保留已有待办编号，避免后续重试重复创建。

## 善达知枢 v0.5.1（2026-09-18）

- 完善会议纪要后台任务、跟进保存和会话切换；生成或引用校验失败时区分具体原因并自动有限重试。
- 架构图问题支持基于文字证据生成带引用的示意图，并明确标注非原始架构图。

## 2026-09-18 知识问答校验与图示回答

- 区分无可用证据、模型判断依据不足、生成格式错误与引用校验失败；记录具体原因，失败结果不再写成“暂无足够可靠资料”。
- 生成及校验失败自动重试一次，清空失败轮次的流式预览；授权或配置错误不重复请求。
- 架构图、流程图请求同时检索文字结构依据，允许生成有引用的示意图并明确区分原图。

## 2026-09-18 会议跟进与后台任务

- 负责人选择接入当前飞书企业通讯录部门树，支持中文、英文、全拼和首字母查找，允许选择未登录助手的成员；分配时校验企业目录，保持访问权限隔离。
- 截止日期改为日期控件，模糊期限保留为建议；知识更新建议明确标注尚未逐条核对正式知识、尚未提交维护队列。
- 会议后台运行时可切换或新建会话；新增站内任务进度、未读结果及完成提醒，保留刷新恢复和取消能力。

## 善达知枢 v0.4.0（2026-09-18）

- 汇总会议纪要技能、统一模型配置及长会议分析重试修复，作为会议跟进功能开发前的版本基线。
- 产品版本通过 Git 标签记录；底层 Yuxi 包版本仍为 0.7.1，保持 CLI 与镜像兼容。

## 2026-09-17 统一模型配置

- 模型供应商页面集中管理接口、凭证及模型用途，善达知枢与企业知识助手共用配置。
- 对话、会议、方案与智能体忽略旧会话和智能体的模型覆盖；快速响应、重排及内容审查按统一用途调用。
- 图谱抽取、思维导图、示例问题与描述生成读取最新统一业务模型；移除图谱独立模型选择。
- 无效分配拒绝保存，持久化失败保留原配置；已有知识库继续使用原向量模型。

## 2026-09-17 会议纪要技能

- 修复长会议汇总因模型 JSON 格式异常直接失败：约束 JSON 输出、有限重试并识别截断；失败显示具体原因，同一资料重试复用已保存的分段笔记，继续严格核验引用。
- 新增 `@会议纪要`，兼容 `@分析会议`；支持公开分享链接、粘贴文字、TXT、Markdown、DOCX 与可提取文字的 PDF。
- 通过 LangGraph 与 LLM 分段整理全文、核对决定和行动项并生成业务分析，区分原始转写、平台总结、助手建议与正式知识依据。
- 听悟、BuddyNote 采用专用读取路径；其他公开会议网页通过服务端浏览器自动尝试识别与展开，无法验证全文或存在访问限制时提示上传文字资料。
- 支持后台进度、刷新恢复、取消与重试、独立历史会议引用、正文自动保存及当前版本 Word 导出；重跑保留原有成功结果和人工修改。
- 更新技能说明、浅色功能架构图及会议纪要实测截图。

## 2026-09-16 产品使用手册

- GitHub 项目首页和文档站首页补充企业知识助手、善达知枢管理端访问网址。

- 新增善达知枢管理员与审核人员手册，更新企业知识助手员工手册；提供两张功能架构图和六张使用演示数据的实际界面截图。
- 首页与导航增加双产品手册入口，说明授权接入、扫描与加工状态、重新加工、重复处理、引用核验和反馈操作。
- 修正文档中旧页签名称、扫描与重新加工的区别，以及重复关系处理后的审核行为。

## 2026-09-15 素材重新加工

- 版式审核直接展示待确认原文、原因和下一步，区分确认纳入、内容重叠及检查等待/失败；原文标记通过内容核验，避免历史片段关联把无关文字标成待确认。

- 跨文档文字比对排除图片文件名、地址和预览路径，保留有效 OCR 正文；旧片段编号不再作为幻灯片定位依据，定位失败明确提示，证据标签与原文预览使用同一份核验页码。

- 文档内图片按透明/纯色、细分隔线、重复页边小图、低色彩复杂度小图标过滤；保留带 OCR 文字、图注、客户标识集中展示页和不确定图片。规则使用 Docling 页面位置及图片内容，不按文件名误删。
- 待审核素材可单条或批量重新加工，从归档原文件新建解析记录，完成后重新检查并保持待审核；拒绝覆盖已有人工审批或已发布片段。旧文件保留，失效的自动审核项和旧比较关系退出当前队列。

## 2026-09-17 企业知识助手手册校准

- 按 v0.3.6 重绘浅色功能架构图：区分普通问答、查资料和做方案，补充条件确认、知识权限、附件边界及反馈发布关系，并提供 SVG 与高清 PNG。

- 按企业知识助手 v0.3.6 更新全部助手操作截图，替换来源详情、反馈等旧版画面；截图使用实际对话页与演示接口数据。
- 补全 `@查资料` 的检索、摘要、原文、下载与分发步骤，以及 `@做方案` 的条件确认、草稿核对、保存、确认和异常处理说明；新增技能选择、资料结果、条件确认和方案草稿截图。
...
## 善达知枢 v0.3.0 (2026-09-05)

### 飞书知识数据源接入

- 审核区增加跨文档检查等待、进行中及失败提示和状态刷新入口，批量审核提示区分检查等待与实际风险。 检查状态每 10 秒自动刷新，编辑时暂停；服务启动恢复遗留的排队检查，避免重启后永久等待；扫描批次在资料页每 10 秒同步实际加工状态。

- 知识加工支持粘贴飞书知识库页面链接，自动解析节点；默认使用平台索引模型自动创建知识库，授权后读取飞书名称，按需展开选择已有知识库。支持页面及下级或整个空间。
- 自动创建的知识库默认仅创建者可见，同名时添加区分后缀；重新授权不重复创建或覆盖已有知识库，内容仍须审核后发布。
- 修正 Milvus 未复用 MinIO 自定义凭据导致反复重启、阻断建库的问题，保持两者连接配置一致。
- 顶部增加数据源管理与切换入口，复用管理员授权；授权完成自动检查连接，首次全量扫描后开放增量扫描。
- 切换数据源时清理旧目录、筛选和审核状态，扫描期间禁用切换与重新授权，保留现有扫描及审核发布链路。

### 企业级方案智能体与执行过程

- 新增基于 LangGraph Agent Run 的 `@做方案` 能力，支持需求分析、企业能力匹配、方案架构、知识增强和质量检查，输出可编辑的方案蓝图草稿。
- 增加正式知识、会话附件、引用、能力边界、冲突和待确认项的结构化处理；证据不足或冲突未解决时明确阻断，不虚构企业能力。
- 修复结构化结果提取、空结果和阻断结果展示，保证运行完成后正文、执行摘要、章节和引用不会出现空卡片。
- 增加安全的执行轨迹持久化、SSE 流式事件、断线恢复、resume/cancel 和历史会话回放；阶段顺序单调，过程信息在刷新后仍可查看。
- 保持当前用户权限边界与 `@查资料` 原有链路不变。

## 善达知枢 v0.2.2 (2026-09-04)

### 企业知识助手资料检索

- 修复助手前端发送 `skillId` 时被后端旧请求模型拒绝的问题，兼容 `MATERIAL_SEARCH`、`SOLUTION_DRAFT` 和 `MEETING_ANALYSIS` 技能标识。
- `@查资料` 命令仅保留在会话记录中，实际检索和回答使用清理后的问题，避免命令文本干扰知识召回。
- 资料检索回答返回当前已审核发布且用户有权限访问的正式资料卡片；引用失效或权限变化时不展示过期资料，并提供受保护的下载与分享准备接口。
- 方案运行多轮补充信息恢复时保留完整回答链；兼容历史运行记录中的附件字段，避免恢复后遗漏用户已提供的上下文。
- 方案智能体运行进度改为单向推进，并与正文流式输出彻底分离：执行阶段仅进入可折叠过程，正文只发送真实方案内容；完成后立即返回已校验的结构化方案，避免展示半截 JSON、伪造阶段文案或内部推理内容。

## 善达知枢 v0.2.1 (2026-09-04)

### 审核工作区与治理体验

- 审核结果拆分为“发布”“退回修改”“不纳入”独立按钮；退回及按适用范围拆分等需要补充信息的操作采用二次确认，减少误提交。
- 审核意见、问题标签、资料责任人和适用范围支持自动保存；切换知识单元或关闭抽屉前会冲刷待保存内容，并绑定原审核包、知识单元和输入快照，避免快速操作串写或丢失。
- 已处理知识单元立即锁定为只读，审核包在全部单元完成前继续保留并显示剩余待处理数量；待处理定位会同步高亮文档页、Excel 工作表或 PPT 幻灯片。
- 审核队列支持文档名称、拼音全拼和拼音首字母搜索，服务端结果使用稳定排序；大屏布局按视口高度自适应，减少底部留白。
- 页面初始化及全量/增量扫描期间持续显示进度，并锁定会产生状态竞争的写操作；切换菜单后仍可查看当前进度和阶段。

### 发布说明

- 本版本为善达知枢治理自动化正式发布，包含 `v0.2.0-rc.1`、`v0.2.0-rc.2` 已验证的运营待办、质量门禁、通知观察模式、加工重试、反馈回流、Markdown 加工、跨文档重复治理及多格式证据定位能力。

## 善达知枢 v0.2.0-rc.2 (2026-09-03)

### 知识加工体验与重复治理

- 扫描进度在切换菜单后继续全局显示，并在知识加工页区分初始化、资料扫描和资料加工排队阶段；扫描期间允许查看资料，但统一暂停审核、重建索引和跨文档补检等写操作，避免并行修改产生状态竞争。
- Markdown 文件可通过上传和飞书知识源进入加工流程，支持正文解析、审核版式展示和跨文档版式对比。
- 跨文档重复裁决选择规范来源后，另一来源中完全由重复片段组成且没有其他未决关系的知识单元自动标记为“重复来源”；含独有片段的单元继续待审，整篇均为重复来源时自动完成对应审核包。审核队列同步显示重复来源数量和完成结果，裁决前后明确提示剩余待审数量及是否无需再次审批；“分别保留”和规范来源自身仍保持原审核流程。
- 修复审核抽屉层级、资料队列分块兜底和扫描批次完成后仍有资料加工任务时的进度衔接问题。
- 审核工作区将发布、退回修改和不纳入拆分为独立操作按钮；退回及其他需要补充信息的结果采用“先选择、再确认”，审核意见、问题标签、资料责任人和适用范围自动保存。已处理知识单元会立即锁定，单条处理后审核包继续保留并显示剩余待处理数量；切换单元或关闭抽屉前会冲刷尚未触发的自动保存，避免快速操作造成意见丢失。
- 处理记录升级为完整业务审计时间线：合并资料变更、审核、跨文档裁决、解析/发布、下架/恢复、重试及知识单元治理元数据操作；跨文档事件同时关联两侧资料，并展示资料标题、版本、操作人和事件说明。负责人、有效期、复核时间等治理元数据修改会记录更新前后值和变更字段。

### 发布说明

- 本版本为 `v0.2.0` 第二个候选版本；后端治理路由、前端审核工作区及相关扫描和 Markdown 测试已通过。真实飞书文档、PDF、Word、PPT、Excel、Markdown 和图片的完整端到端验收完成后，再发布正式 `v0.2.0`。

## 善达知枢 v0.1.6 (2026-09-01)

### 修复

- 优化飞书来源文件版本变化、全部知识单元文字均未变化时的审核处理：页面明确区分“仅检测到文件版本变化”和普通正文变更，提示继续核对图片与版式，并允许审核人员确认无需更新后保留当前正式知识、关闭候选版本且不重复发布或重建索引。
- 混合变化资料和正文解析失败资料不会出现“确认无需更新”操作，避免遗漏真实内容变化或把解析异常误判为无业务变化。

## 善达知枢 v0.2.0-rc.1 (2026-09-02)

### 治理自动化 v1

- 新增“运营待办”入口：聚合待审核、来源更新、跨文档冲突、加工失败、到期复核和助手反馈，支持负责人、类型、风险、状态及超期筛选，并可从待办直接跳转到现有处理页面。
- 为审核包增加质量门禁、五维质量评分、受影响知识摘要和自动关闭资格；正文/解析失败、来源位置缺失、未解决冲突或来源失效时硬性阻断发布。
- 修复资料队列在审核包已生成知识单元但索引分块统计尚未写入时显示为 0 的问题，改为使用当前版本的有效知识单元数作为阶段性统计；全量和增量扫描新增扫描、资料加工排队两阶段的实时进度与当前处理资料提示。知识加工工作台首次加载期间锁定下方操作，等待所有初始化请求结束后再恢复，避免并行请求失败时产生状态竞争。
- 新增治理通知投递记录，保留接收人、渠道、幂等键、状态、失败原因、重试次数和已读时间；默认使用观察模式，不自动关闭任务、不发送真实飞书通知，并支持按 `observe → in_app → feishu → active` 分阶段启用。阶段提升会激活历史通知意图，自动关闭仅处理满足全部安全条件且不超过 500 个知识单元的来源版本审核包。
- 增加资料加工失败自动重试（1、5、15 分钟，最多 3 次）及耗尽后的人工待办；助手点踩支持原因分类和补充说明，并将可定位到正式知识的问题回流治理待办。

### 开发基线

- 本候选版本从善达知枢 `v0.1.6` 基线开发，已完成自动化测试、生产观察模式和扫描进度回归验证；完整多格式端到端验收完成后，再发布正式 `v0.2.0`。

## v0.7.1 (2026-07-17)

### 安全

- 修复 2026-08-27 管理端安全扫描中的接口与镜像级低危及以上问题：用户和对话列表增加分页上限，所有密码登录失败返回一致响应，日志接口限制日志级别并停止返回服务器路径和底层异常；公开健康检查迁移为只返回最小状态的 `/api/system/healthz`，原健康详情要求登录。生产 Nginx 统一增加 CSP、`nosniff` 等安全头，移除 `Server` 指纹和不可信转发 IP，并新增带 HTTP 强制跳转的 HTTPS Compose 覆盖配置；明文登录问题需在目标服务器配置受信任证书、启用该覆盖配置并重新扫描后关闭。
...
- 完成飞书知识加工与治理闭环：知识源读取改为企业自建应用下的用户 OAuth 授权，管理页支持扫码与浏览器授权、空间根目录树缓存、全量/增量扫描及来源失效下架；扫描范围覆盖 Wiki/Docx、PDF、Word、PowerPoint、Excel 和图片 OCR，音视频暂只登记来源。解析后新增跨文档比较、重复/重叠/冲突/证据不足关系、发布/需要修改/驳回/转交审核决策、适用范围、正式知识版本和来源关系，并将审核工作台、关系检查和正式知识直接整合进原有飞书知识加工页面；审核任务默认展示候选资料的解析正文，可切换查看跨文档证据，避免无比较关系时无法核对待审核内容；页面首次加载即同步真实待审核数量，无需点击页签后再回填。审核工作区进一步改为审核包队列，按审核项展示场景化结果、资料修改轮次、来源责任人和事件时间线；飞书新版本按内容 Hash 自动关闭旧修改任务并重新打开关联审核，后台新增资料修改任务查询与取消能力。
- 新增知识单元级局部审核能力：在原始材料与来源片段之上增加版本化知识单元模型，按章节、页面、幻灯片和工作表等结构确定性归并来源片段；审核包自动转换为知识单元审核项，支持同一材料局部纳入、局部排除、沿用已有知识和退回修改，发布时只索引已纳入的来源片段。单个知识单元审核通过后立即进入部分发布，无需等待整份材料完成审核，材料内其他单元继续保持待处理；工作台同步展示已处理、待处理、已纳入进度，并提供“已处理”入口用于查看历史裁决。审核工作台默认优先展示冲突、重叠、变更和证据不足等需要人工确认的单元，同时提供知识单元处理概览、单元级新旧正文对比、来源定位及低风险建议批量采用；PPT 同一页的所有片段保持为一个可完整查看和定位的知识单元。知识加工页将数据源与素材统计压缩为可展开的一行摘要，并把“资料与扫描”移到模块导航末尾；页面会优先恢复最近一次成功加载的工作台数据，短时间内重复进入不再重复请求，目录缓存延长并在扫描完成或手动刷新时强制失效。PPT 版式预览改为按版本缓存版式、PDF 转换结果和页面图片，前端保留最近访问页面并预加载下一页，避免切换每页都重复转换整份 PPT。
- 完善知识单元审核结果管理：审核包默认定位仍在等待资料修改的知识单元；“批量不纳入”可在同一事务内关闭相关修改任务、记录知识单元不纳入结果并同步来源片段发布状态；已取消修改的知识单元明确显示“修改任务已结束”，不再使用含义模糊的“已失效”；任务卡片同步显示已纳入、不纳入和待处理数量，“已完成”任务可按全部纳入、部分纳入和全部不纳入筛选。
- 补齐已发布知识生命周期治理：正式知识支持知识单元下架、恢复、负责人及有效期维护，来源资料支持整篇下架、恢复和回滚到已审核历史版本；索引重建成功后才切换生命周期和活动版本，失败时保留原状态与旧索引。已发布且来源未变化的飞书材料支持单篇或批量重新解析并重建索引，以应用新版图片提取等解析能力；重建期间使用候选文件，成功后原子切换，失败时继续使用旧索引。知识修订继续回到飞书源文档处理，重复发起复用开放任务；企业知识助手收到“不满意”反馈时会根据回答引用定位知识单元并生成去重的纠错任务。
- 修复待审核资料的跨文档检测可能漏掉已发布正式知识：所有当前有效的正式知识不再受最近 200 个候选版本的扫描上限影响；候选资料在数量截断前批量读取解析正文，优先按同一资料版本链与正文相似度筛选，标题和目录仅作次要排序；新增超过元数据候选与扫描上限时仍能命中较早正式知识的回归测试。
- 修复同一飞书文件仅改名或更新时间变化、正文未变时重复创建待审核版本并在跨文档检查中与自身历史版本形成“完全重复”的问题：扫描确认内容哈希一致后复用原素材版本并刷新最新来源元数据；跨文档候选排除相同来源项和已失效来源，历史遗留的同来源或已失效来源关系在补检时自动失效；关系列表分别标记两侧资料的“待审核”“已发布”等状态，明确展示待审知识与正式知识之间的对比；从关系列表进入审核任务时，已处理但未失效的关系仍会定位到对应知识单元供证据查看，但不再影响待审核建议和风险标签；PPT 版式对比在片段定位页未命中证据文字时，会继续匹配其他真实页面并纠正页码和高亮，避免分段页码偏差导致右侧证据无法在左侧找到；跨文档工作区支持网页全屏，并支持两侧版式同步或独立缩放、拖动与复位；放大时按缩放档位加载高清页面，普通滚轮保留页面滚动，仅组合键滚轮和触控板捏合执行缩放，避免 Mac 与 Windows 的滚动操作冲突；Word 与 Excel 对比中的工作表按自然列宽横纵滚动，不再把长文本压缩成省略内容，匹配或点击单元格时同步展示单元格地址和完整原值。
- 新增面向业务用户的独立企业知识助手：使用飞书 OAuth 登录，固定检索指定的正式飞书知识源，支持 `SUPPORTED`、`INSUFFICIENT`、`CONFLICTING` 三态回答、权限内引用及来源跳转；问答界面与 ZhiShu 技术工作台隔离，提供“简洁 / 详细”两种回答模式，简洁模式沿用一次混合检索与流式生成，详细模式通过受限 LangGraph 调查器多步检索、展开原文和定位章节；两种模式都只使用已审核发布、当前有效、无未决冲突且用户可访问的资料，并在最终生成前再次校验引用。问答服务同时加入多轮上下文、重复证据去除、Markdown 答案及真实阶段 SSE；独立界面提供可点击行内来源、答案点赞/点踩、长页面回到底部和移动端来源抽屉，并补充会话、引用、权限边界及真实知识链路的自动化测试和运维说明。飞书登录进一步支持基于企业通讯录的首次自动开户：以企业凭证核验员工身份和在职状态，自动创建普通账号，建立稳定身份绑定，保存全部部门成员关系，并在每次登录时刷新姓名、头像和组织信息；其他企业、停用员工、已删除账号及撤销绑定继续默认拒绝。
- 修复飞书知识发布后的检索引用回填：Milvus 查询会按知识库和文件批量读取必要来源字段，补充飞书来源 URL、Wiki 路径、素材版本和页码信息；回填前清理旧引用并限制字段白名单，避免普通文件或跨知识库结果携带错误引用。
...
- 优化 Agent 上下文压缩：ZhiShu 的 DeepAgents summary adapter 在生成 summary 与写入 conversation history 时，会先对本次模型调用的临时消息视图执行 L1 结构精简，截断旧 `write_file`/`edit_file` 大参数，并把超过阈值的大 `ToolMessage.content` 写入 `outputs/large_tool_results` 后替换为路径和有限预览；L1 不修改 LangGraph state 原始消息，L1 后若上下文低于入口阈值的 40% 则直接调用模型，不生成 summary event，仍超过时才进入 L2 summary。L2 继续使用 DeepAgents `_summarization_event.cutoff_index` 重建 effective messages；Summary 阈值判断改为使用 ZhiShu 自己的近似 token 计算结果，不再根据 provider `usage_metadata.total_tokens` 或 usage scaling 提前触发；首次写入 `conversation_history` 前读取旧文件的 sandbox 404 会按 `file_not_found` 处理，不再产生误导性 warning；`present_artifacts` 会拒绝展示 `large_tool_results` 与 `conversation_history` 等工具调用阶段文件。新增管理员可配置项 `summary_keep_messages`、`summary_prompt`、`summary_tool_result_token_limit` 与 `max_execution_steps`，分别控制摘要后保留消息数、摘要提示词、summary 阶段工具结果预览上限和 LangGraph `recursion_limit`。
...
- 新增外部系统 Agent 调用入口：独立 `agent-invocation` router 提供 `POST /api/agent-invocation/agent-call/runs` 与 `POST /api/agent-invocation/agent-call/runs/result`，字段沿用 ZhiShu 命名（`agent_slug/thread_id/request_id/model_spec`），复用 AgentRun 队列和结果读取能力；支持非流式同步等待或 `async_mode=true` 立即返回 `run_id`，Agent Call 不允许通过 `agent_call_meta.context` 覆盖 Agent context，运行时模型覆盖只允许走独立 `model_spec`；修复无 `thread_id` 且模型校验失败时提前提交空对话，导致孤儿对话和 `request_id` 失败重试非幂等的问题；Agent Call 的 `messages[].content` 兼容 OpenAI 风格的 `text`/`image_url` 多模态数组，纯文本数组不再误报 422，图片输入会保留原始 LangChain 多模态消息供 AgentRun worker 恢复；Agent Eval 与 Agent Call 统一通过 conversation-backed invocation helper 创建 run，后续定时任务等入口只需做请求解析和结果出口适配。
...
- 修复子智能体流式事件兼容：ZhiShu task middleware 的 DeepAgents 子智能体 transformer 改用专用 `yuxi_subagents` projection，避免与 LangChain `create_agent` 默认注册的 `subagents` projection 冲突导致运行流式消息时报错；子线程路由收集优先读取 ZhiShu projection，并保留原 `subagents` fallback。
...
- 补充 Langfuse 集成方案文档：明确采用“云端优先、先 tracing 后 feedback”的接入路径，并约定 ZhiShu 的 `user/thread` 到 Langfuse `user_id/session_id` 的映射关系
